...
export default function NotificationWidget({onClick, notification}: {onClick: () => void, notification: Notification}) {
  const maxChars = 70
  let text: string;
  ...
  return (
    <div className={styles.widget} onClick={onClick}>
      ...
      <p className={styles.content}><span>{text}</span> Click for more</p>
      </div>
    </div>
    )
}